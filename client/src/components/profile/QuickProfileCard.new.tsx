...
import { profileService, UserProfile, ProfileError } from '@/services/profileService.new';
...
export function QuickProfileCard({
  ...
  const tierLevel = profile?.tier || (profile?.totalWager ? getTierFromWager(profile.totalWager) : undefined);
  ...
  return (
    <Card className={cn('overflow-hidden', 
      ...
      <div className="bg-[#1A1B21] p-4 relative">
        {/* Verification Badge */}
        {profile.isVerified && (
          <Badge variant="success" className="absolute top-2 right-2 px-2 py-1 flex items-center gap-1">
