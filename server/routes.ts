  ...
  app.get("/api/users/search", 
    ...
    async (req, res) => {
      try {
        ...
        // Support both 'q' and 'username' parameters for backward compatibility
        const query = (req.query.q || req.query.username) as string;
        console.log("Parsed query:", query, "Length:", query?.length);
        
        console.log("### CHECK: Required length check - query has", query?.length, "chars, minimum required is 3");
        if (!query || query.length < 3) { // Increasing minimum length to 3 characters
          return res.status(400).json({ 
        ...
        const totalCount = parseInt(countResults.rows[0]?.total || '0', 10);
        ...
        // Format response with pagination information to match what the client expects
        const response = {
          results: enhancedUsers,
          pagination: {
            total: totalCount,
            page,
            limit,
